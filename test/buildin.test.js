...
describe("buildin", () => {
    it("extract title use text() and css()", () => {
        ...
        expect(buildin.evalBuildInScript($, script)).to.be.an("string").and.that.equal("微视频 | 愿得此身长报国");
    });
    it("extract title which text() and css() which does not match", () => {
        let script = "text(css('.does .not .exist'))";
        expect(buildin.evalBuildInScript($, script)).to.be.an("string").that.empty;
    });
    ...
    it("extract pubdate in specify area wich does not match", () => {
        ...
        expect(buildin.evalBuildInScript($, script)).to.be.an("string").and.that.empty;
    });
    ...
    it("extract origin  use text() and css()", () => {
        ...
        expect(buildin.evalBuildInScript($, script)).to.be.an("string").and.that.equal("央视新闻客户端");
    });
    it("extract title html use html() and css()", () => {
        let script = "html(css('h1'))";
        expect(buildin.evalBuildInScript($, script)).to.be.an("string").and.that.equal("<h1>微视频 | 愿得此身长报国</h1>");
    });
});